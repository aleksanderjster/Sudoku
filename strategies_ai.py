...
def get_candidate_dict(candidates):
    candidates_dict = {}

    for row in range(9):

        for col in range(9):
            candidates_dict[(row, col)] = set(candidates[row][col])
...
def main():
    grid = last_resolved
    candidates = get_candidate_dict(last_resolved_candidates)
    status = find_xyz_wing(grid, candidates)
